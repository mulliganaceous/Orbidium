...
function drawCenter(canvas) {
    ...
    drawDot(ctx, canvas.width / 2, canvas.height / 2, 5);
}


function drawOrbit(ctx, canvas, radius, textLoc = 0, planetName = '') {
    ctx.arc(canvas.width / 2, canvas.height / 2, radius, 0, 2 * Math.PI, true);
    ctx.stroke();
    ctx.font = "7px Arial";
    ctx.fillText(planetName, (canvas.height / 2) - (textLoc * 4), canvas.height / 2 - (textLoc * 4));
}

function drawOrbitEllipse(ctx, canvas, radiusA, radiusB, textLoc = 0, planetName = '') {
    ctx.ellipse(canvas.width / 2, canvas.height / 2, radiusA, radiusB, Math.PI / 4, 0, 2 * Math.PI);
    ctx.stroke();
    ctx.font = "7px Arial";
    ctx.fillText(planetName, (canvas.height / 2) - (textLoc * 4), canvas.height / 2 - (textLoc * 4));
}
...
function draw() {
    ...
    console.log("Planets:");
    for (i in planets) {
        drawOrbit(ctx, canvas, planets[i].radius * 3, i, planets[i].planet);
    }
    console.log("NEOs");
    for (i in neos) {
        drawOrbitEllipse(ctx, canvas, neos[i].radius_a * 3, neos[i].radius_p * 3, i, neos[i].name);
    }
